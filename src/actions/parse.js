...
export const SHOW_UNKNOWN_POWERMETER_MODEL_MODAL =
  'SHOW_UNKNOWN_POWERMETER_MODEL_MODAL';
export const SHOW_UNKNOWN_SMART_TRAINER_MODEL_MODAL =
  ...
  return dispatch => {
    ...
    setTimeout(() => {
      ...
      Parser.epochify(Parser.normalize(log), (err, normalizedLog) => {
        ...
        try {
          activityData = parseActivity(normalizedLog);

          graphicsData = parseGraphics(normalizedLog, activityData.duration);
          ...
          profileName = systemData.profile
        ...
        if (!isDemo && share && antData.devices) {
          ...
          const powerSourceDevices = _.filter(antData.devices, d => {
            return (
              d.type === SMART_TRAINER_DEVICE || d.type === POWER_METER_DEVICE
            );
          });
