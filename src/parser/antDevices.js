...
export default function antDevices(lines) {
  ...
  _.some(pairings, d => {
    ...
    const matches = d.match(/^\[[^\]]*\]\s+?ant\s+?:\s+?pairing\sdeviceid\s([\d]*)\sto\schannel\s([\d]*).*$/i)

    if (!matches) {
      console.log('Failed to extract deviceId and channel from ant pairing deviceId line')
      return
    }
